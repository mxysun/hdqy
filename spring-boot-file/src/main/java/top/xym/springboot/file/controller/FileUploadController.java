package top.xym.springboot.file.controller;
...
import top.xym.springboot.file.utils.MinIoTemplate;
import top.xym.springboot.file.utils.OssTemplate;
...
public class FileUploadController {
    ...
    public String nativeUpload(MultipartFile file, HttpServletRequest request) {
        if (file != null) {
            ...
            // 对上传的文件重命名，比如  abc.123.jpg > rjksjfkdjfdfdffkdsa.jp
    ...
    // 文件上传
    @PostMapping("/minio")
    private String minioUpload(MultipartFile file) {
        if (file != null) {
            String url;
            ...
            try {
                url = minIoTemplate.putObject("test2", "img/" + UUID.randomUUID() + suffixName, file.getInputStream());
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            return url;
